...
class DNS_MODULE:
	...
	def main(args):
		...
		if not DNS_OPTIONS.TARGET_DOMAIN:
			...
			print(f"{TerminalColor.Orange}example 'python3 StealthWeb.py dns -d domain.com'{TerminalColor.Reset}")
			print(f"{TerminalColor.Orange}Type 'python3 StealthWeb.py dns -h' for commands{TerminalColor.Reset}")
			sys.exit()
		else:
...
<html>
	...
	<body>
		<div class="container">
			<h1 class="text-center">StealthWeb v1.0 - DNS enumeration module</h1>
...
Usage:
  python3 StealthWeb.py dns [args]
...
Examples:
	...
	txt output
	use: python3 StealthWeb.py dns -d domain.com -w wordlist.txt -o report.txt

	html output
	use: python3 StealthWeb.py dns -d domain.com -w wordlist.txt -o report.html
